...
export function PatientsPage() {
  ...
  const selectPatient = async (p) => {
    ...
    setEditMode(false);
    try {
      const res = await api.get(`/api/patients/${p.id}/history`);
      setHistory(res.data);
    } catch (e) {}
  };
  ...
  const saveEdit = async () => {
    try {
      await api.put(`/api/patients/${selected.id}`, editForm);
...
export function RegisterVisitPage() {
  ...
  useEffect(() => {
    ...
    api.get('/api/doctors').then(r => setDoctors(r.data.filter(d => !d.is_on_leave)));
  }, []);
  ...
  const handleRegister = async () => {
    ...
    setLoading(true);
    try {
      const res = await api.post('/api/patients/register-visit', {
