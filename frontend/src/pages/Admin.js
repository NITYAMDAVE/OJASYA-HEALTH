...
export function DiseasesPage() {
  ...
  useEffect(() => { api.get('/api/diseases').then(r => setDiseases(r.data)); }, []);

  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await api.post('/api/diseases', form);
...
export function DoctorsPage() {
  ...
  useEffect(() => { api.get('/api/doctors').then(r => setDoctors(r.data)); }, []);

  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await api.post('/api/admin/staff', { ...form, role: 'doctor' });
      setMsg('Doctor added!');
      setShowModal(false);
      api.get('/api/doctors').then(r => setDoctors(r.data));
    } catch (e) { setMsg(e.response?.data?.message || 'Error'); }
  };

  const toggleActive = async (d) => {
    try {
      await api.put('/api/admin/toggle-active', { role: 'doctor', id: d.id, is_active: !d.is_active });
      api.get('/api/doctors').then(r => setDoctors(r.data));
    } catch (e) {}
  };
...
export function StaffPage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await api.post('/api/admin/staff', { ...form, role: tab });
      setMsg(`${tab} added!`);
      setShowModal(false);
