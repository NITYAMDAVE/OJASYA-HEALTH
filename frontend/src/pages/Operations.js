...
export function MessagesPage() {
  ...
  useEffect(() => {
    fetchMessages();
    api.get('/api/staff').then(r => setStaff(r.data.filter(s => !(s.role === user?.role && s.id === user?.id))));
  }, [user]);
  ...
  const markRead = async (id) => {
    try { await api.put(`/api/messages/${id}/read`); fetchMessages(); } catch (e) {}
  };
...
export function DispensePage() {
  ...
  const viewRx = async (p) => {
    try {
      const res = await api.get(`/api/prescriptions/${p.id}`);
...
export function LeavePage() {
  ...
  const save = async () => {
    setMsg('');
    try {
      await api.put(`/api/doctors/${user?.id}/leave`, form);
...
export function BillsPage() {
  ...
  useEffect(() => {
    ...
    if (user?.role !== 'patient') {
      api.get('/api/patients').then(r => setPatients(r.data));
    }
  }, [user]);

  const fetchBills = async () => {
    try { const r = await api.get('/api/bills'); setBills(r.data); }
    catch(e) {} finally { setLoading(false); }
  };
  ...
  const onPatientChange = async (pid) => {
    ...
    try {
      const [histRes, consultRes] = await Promise.all([
        api.get(`/api/patients/${pid}/history`),
  ...
  const viewBill = async (b) => {
    try { const res = await api.get(`/api/bills/${b.id}`); setSelectedBill(res.data); }
    catch(e) {}
  };
  ...
  const markPaid = async (e) => {
    e.preventDefault();
    try {
      await api.patch(`/api/bills/${payBill.id}/payment`, payForm);
      setMsg(`✅ Bill ${payBill.bill_number} marked as paid!`);
      setShowPayModal(false);
      setPayBill(null);
      fetchBills();
      if (selectedBill?.id === payBill.id) {
        const r = await api.get(`/api/bills/${payBill.id}`);
