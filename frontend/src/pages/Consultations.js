...
export function ConsultationsPage() {
  ...
  useEffect(() => {
    fetchConsultations();
    api.get('/api/diseases').then(r => setDiseases(r.data)).catch(() => {});
  }, []);
  ...
  const saveConsult = async () => {
    try {
      await api.put(`/api/consultations/${selected.id}`, editForm);
...
export function PrescriptionsPage() {
  ...
  useEffect(() => {
    ...
    if (user?.role === 'doctor') {
      ...
      api.get('/api/consultations').then(r => setConsultations(r.data.filter(c => c.status !== 'cancelled')));
    }
  }, [user]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.post('/api/prescriptions', { ...form, patient_id: consultations.find(c => c.id == form.consultation_id)?.patient_id });
